...
def run_llava(prompt, image_path=None, max_tokens=150):
    ...
    # LLaVA often includes the prompt in its output. Remove it to get only the answer.
    # The model's actual response starts after the prompt text.
    if prompt in decoded:
        # Extract only the part after the prompt
        answer_only = decoded.split(prompt, 1)[-1].strip()
        return answer_only
...
for idx, (img_id, img_metadata) in enumerate(data_dict.items()):
    ...
    try:
        ...
        print(f"  [Cross Domain]   Model Answer: {ans_cross_domain[:100]}...")
        print(f"                   Ground Truth: {gt_cross_domain}")
        print(f"  [Within Domain]  Model Answer: {ans_within_domain[:100]}...")
        print(f"                   Ground Truth: {gt_within_domain}")
        print(f"  [Extrapolation]  Model Answer: {ans_extrapolation[:100]}...")
